...
{
    public function batchActionExtended(ProxyQueryInterface $selectedModelQuery)
    {
    ...
    public function batchActionDeleteNeverActivated()
    {
        ...
        $nb = $em->getRepository('AkrpJobeetBundle:Job')->cleanup(60);
        if ($nb) {
            $this->get('session')->setFlash('sonata_flash_success',
                sprintf(
                    '%d never activated jobs have been deleted successfully.',
                    $nb)
            );
        } else {
            $this->get('session')->setFlash('sonata_flash_info',
                'No job to delete.');
        }
